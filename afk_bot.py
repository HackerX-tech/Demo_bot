...
LOG_LIMIT = 1 * 1024 * 1024  # 1 MB

def load_bot_data():
    """Load main bot data (owner, sudo, etc.)"""
    try:
        with open("bot_data.json", "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {"owner": None, "sudo_users": []}

def log_command(user, command):
    """Log command usage with timestamp, user ID, and first name."""
    try:
        bot_data = load_bot_data()
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        user_id = user.id
        first_name = user.first_name or "Unknown"
        entry = f"[{timestamp}] {command} — ID: {user_id} — User: {first_name}\n"

        with open(LOG_FILE, "a", encoding="utf-8") as f:
            f.write(entry)

        if os.path.getsize(LOG_FILE) > LOG_LIMIT:
            # Trigger async rotation in background
            import asyncio
            asyncio.create_task(rotate_logs(bot_data["owner"]))
    except Exception as e:
        print(f"[Logger] Error writing log: {e}")

async def rotate_logs(owner_id):
    """Send logs to owner and reset file if exceeds limit."""
    from telegram import Bot
    try:
        bot_data = load_bot_data()
        bot_token = os.getenv("BOT_TOKEN") or bot_data.get("bot_token")
        if not bot_token:
            print("[Logger] Missing BOT_TOKEN.")
            return
        ...
        if os.path.exists(LOG_FILE):
            await bot.send_document(
                ...
                caption="📄 Log file rotated — old logs sent.",
            )
            os.remove(LOG_FILE)
            print("[Logger] Logs rotated & sent to owner.")
    except Exception as e:
        print(f"[Logger] Rotation failed: {e}")

# ================== LOGS COMMAND ==================
async def logs_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner can fetch logs manually using /logs"""
    user = update.effective_user
    bot_data = load_bot_data()
    owner_id = bot_data.get("owner")

    if user.id != owner_id:
        await update.message.reply_text("❌ Only the bot owner can access logs.")
        return
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        try:
            if update.message and update.message.text and update.message.text.startswith("/"):
                log_command(update.effective_user, update.message.text.split()[0])
...
if __name__ == "__main__":
    ...
    tg.add_handler(CommandHandler("afk", afk))
    tg.add_handler(CommandHandler("afklist", afklist))
    tg.add_handler(CommandHandler("botstats", botstats))
    tg.add_handler(CommandHandler("ping", ping))
    tg.add_handler(CommandHandler("broadcast", broadcast))
    tg.add_handler(CommandHandler("pinall",pinall))
    tg.add_handler(CommandHandler("addsudo",addsudo))
    tg.add_handler(CommandHandler("sendto",sendto))
    tg.add_handler(CommandHandler("promote",promote))
    tg.add_handler(CommandHandler("forwardall",forwardall))
    tg.add_handler(CommandHandler("forwardpin",forwardpin))
    tg.add_handler(CommandHandler("replygroup",replygroup))
    tg.add_handler(CommandHandler("replymention",replymention))
    tg.add_handler(CommandHandler("chatmode",chatmode))
    tg.add_handler(CommandHandler("stopchatmode",stopchatmode))
    tg.add_handler(CommandHandler("delete",delete))
    tg.add_handler(CommandHandler("ban", cmd_ban))
    tg.add_handler(CommandHandler("unban", cmd_unban))
    tg.add_handler(CommandHandler("mute", cmd_mute))
    tg.add_handler(CommandHandler("unmute", cmd_unmute))
    tg.add_handler(CommandHandler("accept", accept_request))
    tg.add_handler(CommandHandler("zombie", zombie_cleaner))
    tg.add_handler(CommandHandler("id", id_command))
    tg.add_handler(CommandHandler("info", info_command))
    tg.add_handler(CommandHandler("purge", purge_command))
    tg.add_handler(CommandHandler("complaint", complaint_command))
    tg.add_handler(CommandHandler("feedback", feedback_command))
    tg.add_handler(CommandHandler("showcomplaints", show_complaints_command))
    tg.add_handler(CommandHandler("sendreaction", send_reaction))
    tg.add_handler(CommandHandler("updatedata", updatedata))
    tg.add_handler(CommandHandler("pinto",pinto))
    tg.add_handler(CommandHandler("logs", logs_command))
# ─── Register handlers ─────────────────────────────────────

    tg.add_handler(CommandHandler("whisper", whisper_command))
    tg.add_handler(CallbackQueryHandler(whisper_button_handler, pattern="^whisper_"))	# ─── /whisper command ──────────────────────────────────────
    tg.add_handler(CommandHandler("userdata", userdata))
    tg.add_handler(CallbackQueryHandler(userdata, pattern="^(delete_user|back_userdata):"))
    tg.add_handler(CommandHandler("editdelete", toggle_edit_delete))
    tg.add_handler(MessageHandler(filters.UpdateType.EDITED_MESSAGE, delete_edited_message))

# Messages
    tg.add_handler(MessageHandler(filters.ALL & ~filters.COMMAND, chatmode_relay))
    tg.add_handler(MessageHandler(filters.ALL & ~filters.COMMAND, handle_message))
    # Group join
    tg.add_handler(ChatMemberHandler(new_group_welcome, ChatMemberHandler.MY_CHAT_MEMBER))


WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET")
app = FastAPI()

def guess_render_url():
    """
    Dynamically derives the Render URL at runtime.
    Render exposes the external hostname inside the environment.
    """
    try:
        host = socket.getfqdn()
...
async def startup():
    ...
    # Detect Render URL
    auto_url = guess_render_url()
    if not auto_url:
        print("⚠️ Could not detect Render URL. Webhook NOT set.")
        return

    webhook_url = f"{auto_url}/webhook/{WEBHOOK_SECRET}"
    await application.bot.set_webhook(webhook_url)

    print("🚀 Auto webhook set to:", webhook_url)
